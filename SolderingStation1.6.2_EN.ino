/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*                    Пояльная Станция v 1.6.2-N             */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */


/* 
===== ПИНЫ =====
pin D12 Пассивного пьезоизлучателя
pin D6  Управление оборотами фена, симистора (IRFZ44N)
pin D3  Zero cross Считование импульсов переменного тока и периудов для управления семистором фена
pin D5  Управление мощностью нагревателя паяльника, симистор (IRFZ44N)
pin D4  Реле защиты паяльника
pin D7  Реле защиты фена
pin D8  Управление мощностью нагревателя фена, симистор (BT139-600E)

===== Кнопки =====
pin D10 Концевой переключатель (GerKon) в фене включает/выключает таймер отключения фена
pin D9  Концевой переключатель (GerKon) в паяльнике включает/выключает таймер отключения паяльник
pin D13 Кнопка включения/выключения Паяльника
pin A0  Кнопка включения/выключения Фена

===== Енкодер =====
pin A3 Кнопка выбора
pin A2 Поворот +
pin A1 Поворот -
Если Енкодер выключен то управление с помощью кнопок
pin A3 Кнопка выбора
pin A2 Кнопка +
pin A1 Кнопка -

===== Операционного усилителя =====
pin A6 Операционного усилителя паяльника
pin A7 Операционного усилителя фена
*/
...
// ===== ОПЦИОНАЛЬНЫЕ НАСТРОЙКИ (раскомментировать нужные) =====
//#define NEED_GRAPH                // График температуры в последовательный порт Serial.begin(9600)
//#define LM358                     // Если используется не (AD8552) а (LM358)
...
#define SOLDER_TIMER_ON_STAND       // Если вы хотите активировать паяльник, таймер только на стенде, тогда раскомментируйте его
#define ENCODER                     // Если используется ENCODER, тогда раскомментируйте его 


// ===== ПИНЫ =====
#define pinBuzzer 12   //pin D12 Пассивного пьезоизлучателя
#define pinFanPwm 6    //pin D6  Управление оборотами фена, симистора (IRFZ44N)
#define pinSolderPwm 5 //pin D5  Управление мощностью нагревателя паяльника, симистор (IRFZ44N)
...
//pin D5  Управление мощностью нагревателя паяльника, симистор (IRFZ44N)
//pin D4  Реле защиты паяльника
//pin D7  Реле защиты паяльника
//pin D8  Управление мощностью нагревателя фена, симистор (BT139-600E)

// ===== Кнопки =====
#define sw_HA 10    //pin D10 Концевой переключатель (GerKon) в фене включает/выключает таймер отключения фена
#define sw_S 9      //pin D9  Концевой переключатель (GerKon) в паяльнике включает/выключает таймер отключения паяльник
#define bt_SON 13   //pin D13 Кнопка включения/выключения Паяльника
#define bt_HAON 14  //pin A0  Кнопка включения/выключения Фена

// ===== Енкодер =====
#ifdef ENCODER
#define e_button 15 //pin A3 Кнопка выбора
#define e_dir 16    //pin A2 Поворот +
#define e_step 17   //pin A1 Поворот -
#else // Если Енкодер выключен то управление с помощью кнопок
#define bt_Sel 15   //pin A3 Кнопка выбора
#define bt_Up 16    //pin A2 Кнопка +
#define bt_Dwn 17   //pin A1 Кнопка -
#endif
 
 
#define FIRMWARE_VERSION "1.6.2-N"
...
// ===== ПОРОГИ ЗАЩИТЫ =====
#define min_solder_temp 100       // Минимальная температура паяльника °C
#define max_solder_temp 400       // Максимальная температура паяльника °C
#define min_hotair_temp 20.0      // Минимальная температура фена °C
#define max_hotair_temp 450.0     // Максимальная температура фена °C
#define min_rpm 30                // Минимальные обороты фена 
#define max_rpm 100               // Максимальные обороты фена
#define default_temp 280          // Стандартная температура °C
#define default_rpm 50            // Стандартные обороты фена
 

// ===== Создание экземпляров обектов Bounce =====
// ===== Объекты для устранения дребезга контактов =====
Bounce swHotAir = Bounce();       // Концевой выключатель (GerKon) в фене
Bounce swSolder = Bounce();       // Концевой выключатель (GerKon) в паяльнике
Bounce SolderOnButton = Bounce(); // Включения/выключения Паяльника
Bounce HotAirOnButton = Bounce(); // Включения/выключения Фена

#ifdef ENCODER
Bounce Enc_Button = Bounce();     // ENCODER
#else // Если Енкодер выключен то управление с помощью кнопок
Bounce SelButton = Bounce();      // Кнопка выбора
Bounce UpButton = Bounce();       // Кнопка +
Bounce DwnButton = Bounce();      // Кнопка -
#endif
...
void setup() {
    ...
    pinMode(3, INPUT_PULLUP); //pin Zero cross Считование импульсов переменного тока и периудов для управления семистором фена
    D5_Out; //pin Управление мощностью нагревателя паяльника, симистор (IRFZ44N)
    D5_Low;
    D4_Out; //pin Реле защиты паяльника (реле)
    D4_Low;
    D7_Out; //pin Реле защиты фена (реле)
    D7_Low;
    D8_Out; //Управление мощностью нагревателя фена, симистор (BT139-600E)
    D8_Low;
    D12_Out; //pin Пассивного пьезоизлучателя
    D12_Low;
    ButtonsSetup(); // Установка кнопок


#ifdef NEED_GRAPH        // Включение последовательного порта
    Serial.begin(9600); 
#endif // Если выключен то..         
    ...
    MemRead(); //чтение из памяти
    
    delay_ms(2000);
    
    initDisplay();    // Иницилизация дисплея
    attachInterrupt(1, ZC, FALLING);
}
 
void loop() {
    ScanButtons(); //Сканирования кнопок
...
//Обновить LCD-дисплей с интервалом 100 мс
...
void WorkWithHotAir() {
    ...
    case st_work: {
        /* Установите оборотов фена охладителя (конвертировать от 30-100% до 80-255 ШИМ) */
        analogWrite(pinFanPwm, map(SetHotAirRPM, min_rpm, max_rpm, 80, 255));
 
        ha_error = HADoProtect();
 
        /* Включить реле защиты Фена */
        if (ha_error == 0) D7_High;
 
        /* Регулятор температуры PI Фена */
        HA_PI();
...
/* Рабочая процедура Пояльника */
void WorkWithSolder() {
 
    /* Прочитать значение терморезистора */
    //GetSolderT = A6_Read >> 1;
//Если выбран операционный уселитель LM358
#ifdef LM358
    GetSolderT = getOversampled_S();
#else
//Если выбран операционный уселитель AD8552
    ...
    if ( SolderON ) {
        ...
        /* Включить реле защиты Пояльника */
        if (s_error == 0) D4_High;
 
        /* регулятор температуры P Пояльника */
        S_P();
...
byte HADoProtect() {
    ...
    /* Критическая защита фена: высокое значение перегрева или термопары недействительно или разрыв провода */
    ...
    /* Критическая защита фена: неверное значение термопары или защита от разрыв провода: значение термопары недействительно или короткое замыкание на провода */
    if (GetHotAirT < 10) {
        HAProtectionOut();
        return 2;
    }
    ...
    /* Расширенная защита фена: температура падает / не изменяется, а мощность > 0  */
    /* И */
    /* Усовершенствованная защита фена: повышение температуры и мощность < 0 */
    static byte t_cnt = 0;
    ...
    if (!HA_temp_stable) {
        ...
        if (!ha_ctrl && millis() - prevHAcontrol > 1000) {
            ...
            if (HAPower > 0.0) {
                ...
                /* температура падает или не изменяется  фена*/
...
byte SDoProtect() {
    ...
    /* Критическая защита паяльника: высокое значение перегрева или терморезистора недействительно или разрыв провода  */
    ...
    /* Критическая защита паяльника: значение терморезистора недействительно или короткое замыкание на провода */
    if (GetSolderT < 10) {
        SProtectionOut();
        return 2;
    }
    ...
    /* Расширенная защита паяльника: температура падает / не изменяется, а мощность> 0 */
    /* И */
    /* Усовершенствованная защита паяльника: повышение температуры и мощность <0 */
    static byte t_cnt = 0;
    static byte t_cnt2 = 0;
    ...
    if (!S_temp_stable) {
        ...
        if (!s_ctrl && millis() - prevScontrol > 1000) {
            ...
            if (SPower > 0) {
                /* температура падает или не изменяется паяльника */
                ...
                /* повышение температуры паяльника */
                if (prev_t < GetSolderT && GetSolderT > SetSolderT) {
                    t_cnt2++;
                    if (t_cnt2 == 10) {
                        SProtectionOut();
...
/* Охлаждение нагревателя фена до температуры ниже 60 градусов */
void Cooling() {
...
/* Функция передискретизации Фена */
...
/* Zero cross INT1 Считование импульсов переменного тока и периудов для управления семистором фена */
void ZC() {
    StartTimer1(HeaterOn, ots);
    RestartTimer1();
}
 
/* Импульс для открытия симистора BT139-600E фена */
void HeaterOn() {
    StopTimer1();
    if (HAPower > 0.0) {
        D8_High;
...
/* Вычислить задержку открытия симистора BT139-600E фена  */
...
/* Регулятор Паяльника P с плавным пуском */
void S_P() {
...
#else
/* Регулятор Паяльника P с быстрым пуском  */
void S_P() {
    int TempPower = sKp * (SetSolderT - GetSolderT + 1);
...
/************************************* конец АВТОМАТИЧЕСКОЕ УПРАВЛЕНИЕ СИМИСТОРОМ ***********************************/
...
void ScanButtons() {
    ...
    /*  Концевой переключатель (GerKon) в фене включает/выключает таймер отключения фена */
    ...
    /* Концевой переключатель (GerKon) в паяльнике включает/выключает таймер отключения паяльник - (только на подставке) */
    if (swSolder.update()) {
    ...
    /* Выключатель подставки для паяльника простой проверки активности (GerKon) */
    ...
    /* Кнопка фена «включено-выключено» */
